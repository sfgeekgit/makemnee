...
Handles:
- GET /api/bounties - List open bounties (with 1-hour delay)
...
def list_bounties(db: Session = Depends(get_db)):
    """
    List all OPEN bounties that are older than 1 hour.

    This 1-hour delay is intentional to encourage agents to use
    blockchain event listeners for real-time discovery instead of
    polling this API endpoint.

    Returns:
        List of open bounties created more than 1 hour ago,
        ordered by creation time (newest first)

    Note:
        - Only returns bounties with status=0 (Open)
        - Bounties less than 1 hour old are excluded
